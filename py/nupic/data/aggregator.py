...
class Aggregator(object):
  ...
  def __init__(self, aggregationInfo, inputFields, timeFieldName=None,
    ...
    - inputFields: The fields from the source. This is a list of tuples as
          returned from the 'fields' member of a RecordStreamIFace. The tuples
          are: (fieldName, type, special). The type can be 'float', 'int', 
          'string', 'datetime' or 'bool'. And the special can be '', 
          'T' (for timestamp), 'S' (for sequenceId), or 'R' (for reset).  
    ...
    if not self._nullAggregation:
      ...
      for (name, type, special) in self._inputFields:
        ...
        if special == 'R' and self._resetFieldIdx is None:
          self._resetFieldIdx = fieldIdx
        if special == 'T' and self._timeFieldIdx is None:
          self._timeFieldIdx = fieldIdx
        if special == 'S' and self._sequenceIdFieldIdx is None:
