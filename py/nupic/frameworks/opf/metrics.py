...
def getModule(metricSpec):
  ...
  elif metricName == 'grokScore_category':
    return GrokCategoryScore(metricSpec)
  elif metricName == 'grokScore_scalar':
    return GrokScalarScore(metricSpec)
...
################################################################################
################################################################################
class _GrokScore(MetricsIface):
  __metaclass__ = ABCMeta

  def __init__(self, metricSpec):
    self.verbosity = 0
    self.frequencyWindowSize = None
    self.movingAverageWindowSize = None
    self.recordHistory = None
    self.steps = 0
    self.frequencies = None
    self.avgScore = None

    params = metricSpec.params
    if params is not None:
      self.verbosity = params.get('verbosity', 0)

      self.frequencyWindowSize = params.get('frequencyWindow', None)
      if self.frequencyWindowSize:
        assert self.frequencyWindowSize >= 1
        self.recordHistory = deque([])
      else:
        raise RuntimeError("Grok score requires a window")

      self.movingAverageWindowSize = params.get('movingAverageWindow', 1)
      self.scoreMovingAverage = _MovingAverage(self.movingAverageWindowSize)
  ...
  def addInstance(self, groundTruth, prediction, record = None):
    ...
    if prediction is not None:
      score = self._compute(groundTruth, prediction, record)

      self.avgScore = self.scoreMovingAverage(score)
    return self.avgScore


  @abstractmethod
  def _compute(self, groundTruth, prediction, record = None):

    """
    TO BE IMPLEMENTED BY SUBCLASSES

    This is the function that does all of the real computation for the grok score.
    It returns the score for the CURRENT (groundTruth, prediction) pair. The
    _GrokScore superclass then takes this and handles the bookkeeping of computing
    moving averages, etc.
    """

################################################################################
class GrokCategoryScore(_GrokScore):
  ...
  def __init__(self, metricSpec):
    ...
    metricSpec.params:
      Dictionary with the following keys:
        ...
        'frequencyWindow':          The size of the moving window over which the
                                    frequency distribution of the ground truth
                                    values is computed

        'movingAverageWindow":      The size of the window over which the moving
                                    average of the Grok Score is calculated.

                                    If this is set to a negative number,
                                    the average will not be windowed and will
                                    be a simple average over all the returned
                                    Grok Scores

                                    If this key is not present, the metric will
                                    compute the Grok Score for the most recent
                                    (input, prediction) pair, without averaging
                                    it with any previous values
    """
    super(GrokCategoryScore, self).__init__(metricSpec)
    self.frequencies = {}

  # =========================================================================
  def _compute(self, groundTruth, prediction, record = None):
    """ Calculates the GrokScalarScore for a new input record."""

    # If we haven't seen this category before, add an entry in the frequency
    # table
    assert self.recordHistory is not None
    score = 0.0

    if prediction not in self.frequencies:
      self.frequencies[prediction] = 0
    if groundTruth not in self.frequencies:
      self.frequencies[groundTruth] = 0

    if len(self.recordHistory) > 0:
      probability = self.frequencies[prediction]/float(len(self.recordHistory))
    else:
      probability = 0.0
    ...
    # Modify history buffers
    self.recordHistory.append(groundTruth)
    self.frequencies[groundTruth] += 1

    if len(self.recordHistory) > self.frequencyWindowSize:
      oldgroundTruth = self.recordHistory.popleft()
      self.frequencies[oldgroundTruth] -= 1
  ...
  # =========================================================================
  def getMetric(self):
    return {'value': self.avgScore, "stats" : {"steps" : self.steps,
                                               "frequency":self.frequencies}}

################################################################################
class GrokScalarScore(_GrokScore):
  """ TODO: Docs
  """
  def __init__(self, metricSpec):
    """
    ...
    metricSpec.params:
      Dictionary with the following keys:
        ...
        'movingAverageWindow":      The size of the window over which the moving
                                    ...
                                    it with any previous values
    """
    super(GrokScalarScore, self).__init__(metricSpec)

  # =========================================================================
  def _compute(self, groundTruth, prediction, record = None):
    """ Calculates the GrokScalarScore for a new input record."""

    groundTruthEncoding = groundTruth
    predictionEncoding = prediction

    assert self.recordHistory is not None
    if self.frequencies is None:
        assert isinstance(groundTruthEncoding, np.ndarray)
        self.frequencies = np.zeros(groundTruthEncoding.shape)

    score = 0.0
    bitsOn = predictionEncoding.sum()

    if bitsOn > 0:
      if len(self.recordHistory) > 0:
        probability = self.frequencies/float(len(self.recordHistory))
      else:
        probability = self.frequencies

      union = groundTruthEncoding * predictionEncoding
      score = (1-probability) * union
      score -= (predictionEncoding - union) * probability

      # Scale by 10 and normalize by w
      score = 10.0 * score.sum()
      score /= predictionEncoding.sum()

    # Modify history buffers
    self.recordHistory.append(groundTruthEncoding.copy())
    self.frequencies += groundTruthEncoding

    if len(self.recordHistory) > self.frequencyWindowSize:
      oldgroundTruth = self.recordHistory.popleft()
      self.frequencies -= oldgroundTruth
