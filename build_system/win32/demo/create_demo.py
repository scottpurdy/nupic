...
def copy_launch_script(script_dir, trunk_dir, work_dir):
  ...
  files = [os.path.join(script_dir, 'DemoLauncher.pyw'),
           os.path.join(vision_dir, 'demo.ico'),
           os.path.join(vision_dir, 'demo_header.bmp'),
           os.path.join(vision_dir, 'demo_left.bmp'),
           os.path.join(vision_dir, 'demo_license.cfg'),
           os.path.join(vision_dir, 'help.html'),
...
def move_nupic_bits(script_dir, trunk_dir, work_dir):
  # Move the RunDemo.py and rename it to Demo.pyw
  vision_dir = os.path.join(work_dir, 'install/share/vision')
  print 'Moving RunDemo.py --> Demo.pyw'
  shutil.move(os.path.join(vision_dir, 'RunDemo.py'),
              os.path.join(work_dir, 'Demo.pyw'))
...
def usage():
  print \
"""Usage: python create_demo.py --install_dir=<install dir> --target=<target filename>
...
def main(argv):
  ...
  work_dir = os.path.join(tmpdir, 'VisionDemo')
  ...
  try:
    # Create the demo release
    create_demo_release(trunk_dir, install_dir, work_dir, version)
    ...
    # Move NuPIC packages, the RunDemo.py, the nta4 network and the data
...
if __name__=='__main__':
  if '-t' in sys.argv:
    argv = [sys.argv[0],
            ...
            '--target=%s/VisionDemo.setup.exe' % os.environ['USERPROFILE'],
            '--debug']
  else:
    #--install_dir=c:/nta/install --target=C:/Users/Gigi/VisionDemo.setup.exe --debug
    argv = sys.argv
