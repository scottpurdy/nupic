...
# log dir is /Volumes/big/www/autobuild/rxxx
# cache dir is /Volumes/big/www/autobuild-cache/rxxx
cacheRoot = "/Volumes/big/www/autobuild-cache"
import re

def get_elapsed(logfile):
    dirname = os.path.dirname(logfile)
    cachedirname = os.path.join(cacheRoot, dirname)
    cachefile = os.path.join(cachedirname, os.path.basename(logfile))
    if os.path.exists(cachedirname):
        if os.path.exists(cachefile):
            elapsedstr = open(cachefile).read()
            return elapsedstr
    else:
        try:
    ...
    sec = elapsed
    elapsedstr = "%d:%02d:%02d" % (hour, min, sec)
    try:
        open(cachefile, "w").write(elapsedstr)
    except:
        pass
    return elapsedstr;
    ...
    for i in xrange(0, nBuilds):
        ...
        for a in ["linux64", "linux32", "darwin86", "win32"]:
            if a in b.status:
                ...
                encodedLogfile = logfile.replace("/", "%2F")
                # url = "%s/%s" % (baseurl, logfile)
                url = "%s/testtimes.py?log=%s" % (baseurl, logfile)
